<script lang="ts">
	// Theme picks are instant (like the top-bar toggle) — no staged Save.
	import { settings, setPreferredTheme, themesModalOpen } from '$lib/stores';
	import { allThemes, resolveTheme, type Theme, type TokenName } from '$lib/themes';
	import ThemeEditorModal from './ThemeEditorModal.svelte';

	const themes = $derived(allThemes($settings));
	const activeThemeId = $derived(resolveTheme($settings).id);
	const preferredDark = $derived($settings?.theme_dark ?? 'dracula');
	const preferredLight = $derived($settings?.theme_light ?? 'paper');
	const SURFACES: TokenName[] = ['bg-deep', 'bg-panel', 'bg', 'bg-raised', 'bg-hover'];
	const SWATCHES: TokenName[] = ['accent', 'danger', 'success', 'info'];

	const pick = (t: Theme) => void setPreferredTheme(t.dark ? 'dark' : 'light', t.id);

	let editorOpen = $state(false);
	let editorInitial = $state<string | null>(null);
	function openEditor(name: string | null) {
		editorInitial = name;
		editorOpen = true;
	}

	const close = () => themesModalOpen.set(false);
</script>

<svelte:window
	onkeydown={(e) => $themesModalOpen && !editorOpen && e.key === 'Escape' && close()}
/>

{#if $themesModalOpen}
	<div
		class="backdrop"
		role="button"
		tabindex="-1"
		onclick={close}
		onkeydown={(e) => e.key === 'Escape' && close()}
	>
		<!-- Stop backdrop clicks inside the dialog from closing it. -->
		<div
			class="dialog"
			role="dialog"
			aria-modal="true"
			aria-label="Themes"
			tabindex="-1"
			onclick={(e) => e.stopPropagation()}
		>
			<header>
				<h2>Themes</h2>
				<button class="close" aria-label="Close" onclick={close}>✕</button>
			</header>

			<div class="theme-grid">
				{#each themes as theme (theme.id)}
					<!-- svelte-ignore a11y_no_static_element_interactions -- role/tabindex/keydown
					     are present; a real <button> can't nest the edit button. -->
					<div
						class="theme-card"
						class:current={theme.id === activeThemeId}
						role="button"
						tabindex="0"
						style="background: {theme.colors.bg}; border-color: {theme.id === activeThemeId
							? theme.colors.accent
							: theme.colors.border}"
						onclick={() => pick(theme)}
						onkeydown={(e) => e.key === 'Enter' && pick(theme)}
					>
						<span class="strip">
							{#each SURFACES as s (s)}<span style="background: {theme.colors[s]}"></span>{/each}
						</span>
						<span class="card-main">
							<span class="card-name" style="color: {theme.colors.accent}">
								{theme.label}
								{#if theme.id === preferredDark}<span title="Your dark theme">🌙</span>{/if}
								{#if theme.id === preferredLight}<span title="Your light theme">☀️</span>{/if}
							</span>
							<span class="chips">
								{#each SWATCHES as c (c)}
									<span class="chip" style="background: {theme.colors[c]}; color: {theme.colors.bg}"
										>あ</span
									>
								{/each}
							</span>
						</span>
						{#if theme.id.startsWith('user:')}
							<button
								class="edit"
								title="Edit theme"
								onclick={(e) => {
									e.stopPropagation();
									openEditor(theme.label);
								}}>✎</button
							>
						{/if}
					</div>
				{/each}
				<button class="theme-card new" onclick={() => openEditor(null)}>＋ New theme</button>
			</div>
			<p class="hint">
				Clicking a dark theme sets your dark pick, a light theme your light pick — the top-bar
				light/dark toggle switches between the two.
			</p>
		</div>
	</div>
{/if}

<ThemeEditorModal bind:open={editorOpen} initial={editorInitial} />

<style>
	.backdrop {
		position: fixed;
		inset: 0;
		display: flex;
		align-items: center;
		justify-content: center;
		background: color-mix(in srgb, var(--bg-deep) 70%, transparent);
		z-index: 50;
	}
	.dialog {
		display: flex;
		flex-direction: column;
		gap: 0.6rem;
		width: min(560px, 92%);
		max-height: 90%;
		overflow-y: auto;
		padding-bottom: 0.75rem;
		background: var(--bg-panel);
		border: 1px solid var(--border);
		border-radius: var(--radius);
		box-shadow: 0 8px 32px rgba(0, 0, 0, 0.5);
	}
	header {
		display: flex;
		align-items: center;
		justify-content: space-between;
		padding: 0.75rem 1rem;
		border-bottom: 1px solid var(--border);
	}
	header h2 {
		margin: 0;
		font-size: 1.05rem;
		color: var(--accent);
	}
	.close {
		padding: 0.1rem 0.4rem;
	}
	.theme-grid {
		display: grid;
		grid-template-columns: repeat(3, 1fr);
		gap: 0.5rem;
		padding: 0 1rem;
	}
	/* Cards render in their OWN theme's colors (inline styles), not the active one. */
	.theme-card {
		position: relative;
		display: flex;
		align-items: stretch;
		gap: 0.5rem;
		padding: 0;
		overflow: hidden;
		border: 2px solid;
		border-radius: var(--radius);
		cursor: pointer;
		text-align: left;
	}
	.strip {
		display: flex;
		flex-direction: column;
		width: 8px;
		flex: none;
	}
	.strip span {
		flex: 1;
	}
	.card-main {
		display: flex;
		flex-direction: column;
		gap: 0.3rem;
		padding: 0.45rem 0.5rem 0.5rem 0;
		min-width: 0;
	}
	.card-name {
		font-size: 0.85rem;
		font-weight: 700;
		white-space: nowrap;
		overflow: hidden;
		text-overflow: ellipsis;
	}
	.chips {
		display: flex;
		gap: 0.3rem;
	}
	.chip {
		display: inline-flex;
		align-items: center;
		justify-content: center;
		width: 1.3rem;
		height: 1.3rem;
		border-radius: 4px;
		font-size: 0.75rem;
		line-height: 1;
	}
	.theme-card .edit {
		position: absolute;
		top: 0.25rem;
		right: 0.25rem;
		padding: 0 0.3rem;
		font-size: 0.75rem;
		background: none;
		border: none;
		opacity: 0.6;
	}
	.theme-card .edit:hover {
		opacity: 1;
	}
	.theme-card.new {
		align-items: center;
		justify-content: center;
		padding: 0.45rem;
		background: var(--bg-raised);
		border: 2px dashed var(--border);
		color: var(--text-muted);
		font-size: 0.85rem;
	}
	.hint {
		margin: 0;
		padding: 0 1rem;
		font-size: 0.85rem;
		color: var(--text-muted);
	}
</style>
