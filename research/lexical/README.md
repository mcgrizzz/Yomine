...
The study uses full JMdict and Jitendex, five independent installed frequency dictionaries,
and installed UniDic over the cached subtitle corpus. Production matching is unchanged.

## Source exclusion

**Never use Anilist Top 500 for generated references, analysis, matching-policy evidence,
or validation. It was generated by Yomine and can reproduce past systemic errors.**
The builder skips it before opening its banks. Analysis and compilation also exclude
it from legacy databases, using the shared `source_policy.py`. Do not treat other
Yomine-generated dictionaries as independent evidence either. Old unfiltered databases
and their frequency totals are superseded by the independent reference below.

## Ready-to-use local artifacts

Artifacts are in the sibling directory `../yomine-lexical-research/` relative to the
main worktree root (`yomine`). They are intentionally not committed.

- `lexical-reference.jsonl.gz`: complete portable reference, about 14.3 MB compressed.
- `lexical-reference-independent.sqlite`: indexed research database, about 518 MB including frequencies.
- `lexical-reference-independent.manifest.json`: source versions, hashes, bank hashes, entity codes and attribution.
- `artifacts-independent.json`: checksums for the corrected database, manifest and audit.
- `report-independent/summary.json`: full-vocabulary counts and source provenance.
- `report-independent/ambiguous_readings.csv`: every ambiguous reading and its entry IDs.
- `report-independent/jitendex_disagreements.json`: source/version/scope discrepancies to review.
...
python3 research/lexical/analyze.py \
  --db ../yomine-lexical-research/lexical-reference-independent.sqlite lookup できる
```
