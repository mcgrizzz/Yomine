...
use yomine::{
    anki::FieldMapping,
    core::{ models::FileType, pipeline::process_source_file, SourceFile },
    dictionary::{ frequency_manager, token_dictionary::DictType },
    gui::{ LanguageTools, YomineApp },
...
async fn main() {
    ...
    model_mapping.insert("Lapis".to_string(), FieldMapping {
        term_field: "Expression".to_string(),
        reading_field: "ExpressionReading".to_string(),
    });

    model_mapping.insert("Kaishi 1.5k".to_string(), FieldMapping {
    ...
    let frequency_manager = Arc::new(
        frequency_manager
            ::process_frequency_dictionaries()
            .expect("Failed to load Frequency Manager")
    );
    ...
    let (terms, sentences) = match
        process_source_file(&source_file, model_mapping.clone(), &language_tools).await
    {
        Ok(result) => result,
        Err(_) => {
    ...
    let _ = eframe::run_native(
        ...
        native_options,
        Box::new(|cc|
            Ok(Box::new(YomineApp::new(cc, terms, sentences, model_mapping, language_tools)))
        )
    );
}
